...
All VSP application manifests are stored in the [vsp-infra-application-manifests repository](https://va.ghe.com/software/vsp-infra-application-manifests), in the [apps/vsp-cms subfolder](https://va.ghe.com/software/vsp-infra-application-manifests/tree/main/apps/vsp-cms).  The [@cms-infrastructure team](https://va.ghe.com/orgs/software/teams/cms-infrastructure) are code owners for the manifests, and all pull requests must be approved by a member of that team.  Once the pull request is merged, Argo CD will deploy it automatically.

### Secrets and Application Configuration Parameters
During a deploy the CMS application needs to know where to find app config parameters as well as parameters or values that are sensitive and must remain secret. As part of the Application Manifest CMS will define an `externalSecrets` resource that is backed by AWS Systems Manager Parameter Store. All CMS related parameters and secrets should be added to Parameter Store and referenced like in [this example](https://va.ghe.com/software/vsp-infra-application-manifests/blob/main/apps/vsp-identity/test-user-dashboard/dev/externalsecrets.yaml).
