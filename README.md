...
Access to the production instance is restricted to those with CAG or proxy access. See [Getting Access](READMES/access.md).

This app is built on Drupal 8, specifically the Acquia Lightning distribution. See [Getting Started](READMES/getting-started.md).

## How WEB & CMS Work Together

The public website seen at VA.gov is generated by the repository located at 
[vets-website](https://github.com/department-of-veterans-affairs/vets-website), known as `WEB` or "the Front-end".

The `WEB` app reads content from the [CMS](https://cms.va.gov) via GraphQL (and other locations) to render the content 
into HTML, which is deployed to [VA.gov](https://www.va.gov).
