...
class BuildSchedulerTest extends UnitTestCase {
  ...
  public function setUp() {
    ...
    // Override the config factory service because DateFormatter gets it directly from \Drupal. :/
  ...
  public function testCheckScheduledBuild($time, $last_scheduled_build_request, $should_request_build) {
    ...
    // If the build was requested, make sure that the timestamp was updated properly.
  ...
  /**
   * Get a mocked build requester that will either require or disallow requesting a build.
   *
   * @param boolean $shouldExpectBuildRequest
   *   Whether or not the build requester should get a request.
   *
   * @return BuildRequesterInterface
   *   (mocked)
   */
