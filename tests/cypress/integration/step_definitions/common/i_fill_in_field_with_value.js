...
Given("I fill in {string} field with fake text", (label) => {
  cy.findAllByLabelText(label).type(faker.lorem.sentence(), { force: true });
});

Given("I fill in field with selector {string} with fake text", (selector) => {
  cy.get(selector).focus();
  cy.get(selector).type(faker.lorem.sentence(), { force: true });
});

Given("I fill in {string} field with fake link", (label) => {
