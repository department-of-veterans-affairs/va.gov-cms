...
/**
 * A Drush interface to the Build Frontend service.
 */
class WebBuildCommands extends DrushCommands {
   ...
   * @param \Drupal\va_gov_build_trigger\Service\BuildFrontendInterface $frontendBuild
   *   The frontend build service.
   */
  public function __construct(
    WebBuildCommandBuilderInterface $commandBuilder,
    BuildFrontendInterface $frontendBuild
  ) {
  ...
  public function buildFrontend(string $reference = NULL, string $fullRebuild = 'FALSE') {
    ...
    if ($options['dry-run']) {
      ...
      $this->frontendBuild->triggerFrontendBuild($reference, $fullRebuild);
    }
  }
