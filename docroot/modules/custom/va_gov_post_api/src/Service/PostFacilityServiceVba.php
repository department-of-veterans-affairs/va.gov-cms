 ...
 */
class PostFacilityServiceVba extends PostFacilityServiceBase {
  ...
  public function queueFacilityService(EntityInterface $entity, bool $forcePush = FALSE) {
    $this->errors = [];
    $queued_count = 0;
    if (($entity->getEntityTypeId() === 'node') && ($entity->bundle() === 'vba_facility_service')) {
  ...
  public function queueServiceTermRelatedServices(EntityInterface $entity) {
    ...
    if (($entity->getEntityTypeId() === 'taxonomy_term') && ($entity->bundle() === 'health_care_service_taxonomy')) {
      ...
      $result = $query->condition('type', 'vba_facility_service')
      ...
      if (!empty($result)) {
        try {
          ...
          while ($current < $total) {
            ...
            $message = sprintf('VA.gov Post API: %s of %d vba_facility_service nodes processed. Queued %s vba_facility_service nodes for sync to Lighthouse.', $current, $total, $queued_count);
            $this->loggerChannelFactory->get('va_gov_post_api')->info($message);

          }
        }
        catch (\Exception $e) {
          $message = sprintf('VA.gov Post API: Failed queuing items of type vba_facility_service. %e', $e->getMessage());
          $this->loggerChannelFactory->get('va_gov_post_api')->error($message);
        }
      }
    }
